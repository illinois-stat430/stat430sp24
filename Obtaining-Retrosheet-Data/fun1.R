parse.retrosheet.pbp <- function(season){
  ...
  ## This function required modification
  create.csv.file <- function(year){
    wd <- getwd()
    setwd("download.folder/unzipped")
    
    cmd_output <- system(paste0('dir ',year,"*EV*",sep=""), intern = TRUE)
    datafiles <- ""
    
    for (i in 1:length(cmd_output)) {
      split_string <- strsplit(cmd_output[i], "  ")[[1]]
      for (j in 1:length(split_string)) {
        datafiles <- paste(datafiles,split_string[j])
      }
    }
    
    shell(paste(paste("cwevent -y", year, "-f 0-96"),
                paste(substr(datafiles,2,nchar(datafiles))),
